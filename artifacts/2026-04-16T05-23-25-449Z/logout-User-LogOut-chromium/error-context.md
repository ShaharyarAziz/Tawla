...
```ts
  ...
  13 |   async logout() {
  14 |     // click avatar
  15 |     await this.avatar.click();
  16 | 
  17 |     // ensure dropdown opens (retry once if needed)
  18 |     const logoutVisible = this.page.locator(".logout-btn:visible");
  19 | 
  20 |     if (!(await logoutVisible.isVisible())) {
  21 |       await this.page.locator("#dropdownToggle").dispatchEvent("click");
  22 |     }
  23 | 
> 24 |     await logoutVisible.waitFor({ state: "visible" });
     |                         ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  25 |     await logoutVisible.click();
  26 |   }
  27 | }
  28 | 
```