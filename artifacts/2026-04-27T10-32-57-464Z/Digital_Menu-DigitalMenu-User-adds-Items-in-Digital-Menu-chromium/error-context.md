...
```
Error: locator.click: Error: strict mode violation: getByText('Vegetarian') resolved to 2 elements:
    1) <span>Vegetarian</span> aka getByText('Vegetarian', { exact: true })
    2) <span>Non-Vegetarian</span> aka getByText('Non-Vegetarian')

Call log:
  - waiting for getByText('Vegetarian')
...
```ts
  ...
  12 |     this.vegeterian = page.getByText("Vegetarian");
