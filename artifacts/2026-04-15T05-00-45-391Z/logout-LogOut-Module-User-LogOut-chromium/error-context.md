...
Call log:
  - waiting for locator('a[href="https://gettawla.ae/merchant/logout"]')
...
```ts
  ...
  14 |     await logout.avatar.click();
> 15 |     await logout.logout_btn.click();
     |                             ^ TimeoutError: locator.click: Timeout 10000ms exceeded.
  16 |     await expect(page).toHaveURL(/\/merchant\/login$/);
  17 |   });
  18 | });
  19 | 
```