...
Locator:  locator('a[href="http://13.51.170.24/merchant/logout"]')
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('a[href="http://13.51.170.24/merchant/logout"]')
    9 × locator resolved to <a class="logout-btn" onclick="localStorage.clear();" href="http://13.51.170.24/merchant/logout">Logout</a>
...
> 21 |     await expect(logout.logout_btn).toBeVisible();
  ...
  23 |     await expect(logout.logout_btn).toBeVisible();
  24 |     await expect(page).toHaveURL("merchant/login");
  25 |   });
  26 | });
  27 | 
```