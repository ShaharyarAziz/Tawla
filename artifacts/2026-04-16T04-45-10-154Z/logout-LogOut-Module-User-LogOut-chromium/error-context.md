...
Call log:
  - waiting for locator('.logout-btn') to be visible
...
```ts
  ...
  19 | 
  20 |     await logout.avatar.click({ force: true });
  21 | 
  22 |     // small wait for animation
  23 |     await page.waitForTimeout(500);
  24 | 
> 25 |     await logout.logout_btn.waitFor({ state: "visible" });
     |                             ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  26 |     await logout.logout_btn.click();
