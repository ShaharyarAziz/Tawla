...
Call log:
  - waiting for locator('.logout-btn:visible') to be visible
...
```ts
  ...
  15 | 
  16 |   const logoutBtn = this.page.locator('.logout-btn:visible');
  17 | 
  18 |   // wait a short time for normal click to work
  19 |   await this.page.waitForTimeout(300);
  20 | 
  21 |   if (!(await logoutBtn.isVisible())) {
  22 |     // fallback: trigger JS event
  23 |     await this.page.locator('#dropdownToggle').dispatchEvent('click');
  24 |   }
  25 | 
> 26 |   await logoutBtn.waitFor({ state: 'visible' });
     |                   ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  27 |   await logoutBtn.click();
  28 | }
  29 | }
  30 | 
```