...
```ts
  ...
  10 |     const link = this.page.getByRole("link", { name: "Digital Menu" });
  11 | 
> 12 |     await expect(link).toBeVisible();
  ...
  20 |     ]);
  21 | 
  22 |     // UI validation (very important)
  23 |     await expect(
  24 |       this.page.getByText("Categories"), // adjust if needed
  25 |     ).toBeVisible({ timeout: 20000 });
  26 | 
  27 |     // Optional strict URL check
  28 |     await expect(this.page).toHaveURL(/digitalmenu\/menu\/digitalMenuSection/);
  29 |   }
  30 | }
  31 | 
```