            ...
            - generic [ref=e98]:
              - generic [ref=e99]:
                - generic [ref=e100]:
                  ...
                  - textbox "Item Name" [active] [ref=e102]: Hot Sandwich
                - generic [ref=e103]:
...
```ts
  ...
  34 | 
  35 |     // 🔥 FOOD TYPE (FIXED)
> 36 |     await this.page.locator("#food-type-label").click();
     |                                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  37 | 
  38 |     const foodMenu = this.page.locator("#food-type-dropdown");
  39 |     await foodMenu.waitFor({ state: "visible" });
  40 | 
  41 |     await foodMenu.getByText("Vegetarian").click();
  42 |     await foodMenu.getByText("Non-Vegetarian").click();
  43 | 
  44 |     // 💰 PRICE
  45 |     await this.item_price.fill(price.toString());
  46 | 
  47 |     // 🔥 CATEGORY (FIXED)
  48 |     await this.page.locator('[data-dropdown="category-dropdown"]').click();
  49 | 
  50 |     const categoryMenu = this.page.locator("#category-dropdown");
  51 |     await categoryMenu.waitFor({ state: "visible" });
  52 | 
  53 |     await categoryMenu.getByText("Recycled Cotton Cheese").click();
  54 | 
  55 |     const marbleHat = categoryMenu.getByText("Modern Marble Hat");
  56 |     await marbleHat.scrollIntoViewIfNeeded();
  57 |     await marbleHat.click();
  58 | 
  59 |     // 🔥 SUB CATEGORY (FIXED)
  60 |     await this.page.locator('[data-dropdown="subcategory-dropdown"]').click();
  61 | 
  62 |     const subMenu = this.page.locator("#subcategory-dropdown");
  63 |     await subMenu.waitFor({ state: "visible" });
  64 | 
  65 |     const spicyFries = subMenu.getByText("Spicy Fries");
  66 |     await spicyFries.scrollIntoViewIfNeeded();
  67 |     await spicyFries.click();
  68 |   }
  69 | }
  70 | module.exports = { items };
  71 | 
```