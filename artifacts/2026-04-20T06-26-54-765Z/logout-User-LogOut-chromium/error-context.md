...
```ts
  ...
  15 | 
  16 |   try {
  17 |     await this.logout_btn.waitFor({ state: "visible", timeout: 3000 });
  18 |   } catch {
  19 |     await this.avatar.dispatchEvent("click");
  20 |   }
  21 | 
  22 |   await Promise.all([
  23 |     // wait for final redirect (not 302)
> 24 |     this.page.waitForURL(/login/, { timeout: 15000 }),
     |               ^ Error: page.waitForURL: Test timeout of 30000ms exceeded.
  25 | 
  26 |     // click logout
  27 |     this.logout_btn.click()
  28 |   ]);
  29 | 
  30 |   // Strong UI validation (VERY IMPORTANT)
  31 |   await this.page.getByRole('button', { name: 'Log in' }).waitFor();
  32 | }
  33 | }
  34 | 
```