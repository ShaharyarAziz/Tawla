...
```
Error: locator.click: Error: strict mode violation: locator('span') resolved to 113 elements:
    1) <span class="font-weight-light mt-2 float-left"> </span> aka locator('span').first()
    2) <span class="branch-select-trigger__text">Angels Sweets-main branch</span> aka getByRole('button', { name: 'Angels Sweets-main branch' })
    3) <span aria-hidden="true" class="branch-select-chevron"></span> aka getByRole('button', { name: 'Angels Sweets-main branch' })
    4) <span>Angels Sweets-main branch</span> aka getByText('Angels Sweets-main branch').nth(1)
    5) <span>Test</span> aka locator('span').filter({ hasText: 'Test' })
    6) <span class="dropdown-toggle"></span> aka locator('span').nth(5)
    7) <span>Angels Sweets1234567</span> aka getByText('Angels Sweets1234567')
    8) <span class="user-email">angels.tawla@gmail.com</span> aka getByText('angels.tawla@gmail.com')
    9) <span class="brand-title">Tawla</span> aka getByRole('link', { name: 'Merchant Logo Tawla' })
    10) <span class="sidebar-icon">…</span> aka getByRole('link', { name: 'Dashboard icon Dashboard' })
    ...
...
```ts
  1  | import { URLS } from "../utils/url";
  2  | export class DigitalMenuPage {
  3  |   constructor(page) {
  4  |     this.page = page;
  5  |     this.digitalMenuLink = page.locator("span", "Digital Menu");
  6  |   }
  7  |   async navigate() {
  8  |     await this.page.goto(URLS.login);
  9  |   }
  10 |   async goToDigitalMenu() {
> 11 |     await this.digitalMenuLink.click();
     |                                ^ Error: locator.click: Error: strict mode violation: locator('span') resolved to 113 elements:
  12 |   }
  13 | }
  14 | 
