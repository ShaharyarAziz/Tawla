...
Locator:  getByText('Vegetarian', { exact: true })
Expected: visible
Received: hidden
Timeout:  10000ms
...
Call log:
  ...
  - waiting for getByText('Vegetarian', { exact: true })
    13 × locator resolved to <span>Vegetarian</span>
            ...
            - generic [ref=e98]:
              - generic [ref=e99]:
                - generic [ref=e100]:
                  ...
                  - textbox "Item Name" [ref=e102]: Spicy Fries
                - generic [ref=e103]:
...
```ts
  146 |     // ---------------- PRICE ----------------
  147 |     this.item_price = page.locator("input#item_price");
  148 | 
  149 |     // ---------------- CATEGORY ----------------
  150 |     this.categoryDropdown = page.locator(
  151 |       'div.custom-dropdown-trigger[data-dropdown="category-dropdown"]',
  152 |     );
  153 | 
  154 |     this.allDesert = page.getByText("All Desert", {
  155 |       exact: true,
  156 |     });
  157 | 
  158 |     this.drinks = page.getByText("Drinks", {
  159 |       exact: true,
  160 |     });
  161 | 
  162 |     // ---------------- SUB CATEGORY ----------------
  163 |     this.subCategoryDropdown = page.locator(
  164 |       '[data-dropdown="subcategory-dropdown"]',
  165 |     );
  166 | 
  167 |     this.subCategoryMenu = page.locator("#subcategory-dropdown");
  168 | 
  169 |     // ---------------- FILE INPUT ----------------
  170 |     this.fileInput = page.locator("#item_image");
  171 | 
  172 |     // ---------------- DESCRIPTION ----------------
  173 |     this.itemDescription = page.locator("#description");
  174 |   }
  175 | 
  176 |   // =====================================================
  177 |   // GENERIC SAFE CLICK
  178 |   // =====================================================
  179 |   async safeClick(locator) {
  180 |     await locator.scrollIntoViewIfNeeded();
  181 | 
  182 |     await expect(locator).toBeVisible();
  183 | 
  ...
  188 | 
  189 |   // =====================================================
  190 |   // OPEN CREATE ITEM MODAL
  191 |   // =====================================================
  192 |   async openCreateItemModal() {
  193 |     await this.safeClick(this.createItemBtn);
  194 |   }
  195 | 
  196 |   // =====================================================
  197 |   // SCROLL & SELECT SUBCATEGORY
  198 |   // =====================================================
  199 |   async scrollAndSelectSubCategory(text) {
  200 |     const container = this.subCategoryMenu;
  201 | 
  202 |     await expect(container).toBeVisible();
  203 | 
  204 |     let found = false;
  205 | 
  206 |     for (let i = 0; i < 15; i++) {
  207 |       const item = container.getByText(text, {
  208 |         exact: true,
  209 |       });
  210 | 
  211 |       if ((await item.count()) > 0) {
  212 |         await item.first().scrollIntoViewIfNeeded();
  213 | 
  214 |         await item.first().click({
  215 |           force: true,
  216 |         });
  217 | 
  218 |         found = true;
  ...
  220 |       }
  221 | 
  222 |       // Scroll inside dropdown
  223 |       await container.evaluate((el) => el.scrollBy(0, 200));
  224 | 
  225 |       await this.page.waitForTimeout(300);
  226 |     }
  227 | 
  228 |     if (!found) {
  229 |       throw new Error(`${text} not found in subcategory dropdown`);
  230 |     }
  231 |   }
  232 | 
  233 |   // =====================================================
  234 |   // SAVE ITEM
  235 |   // =====================================================
  236 |   async saveItem(name, price) {
  237 |     // ---------------- ITEM NAME ----------------
  238 |     await this.item_name.fill(name);
  239 | 
  240 |     // =====================================================
  241 |     // FOOD TYPE
  242 |     // =====================================================
  243 | 
  244 |     await this.safeClick(this.foodtypeDropdown);
  245 | 
> 246 |     await expect(this.vegeterian).toBeVisible({
      |                                   ^ Error: expect(locator).toBeVisible() failed
  247 |       timeout: 10000,
  248 |     });
  249 | 
  250 |     // Select Vegetarian
  251 |     await this.safeClick(this.vegeterian);
  252 | 
  253 |     // Dropdown may close after first selection
  254 |     if (!(await this.nonVegeterian.isVisible())) {
  255 |       await this.safeClick(this.foodtypeDropdown);
  256 | 
  257 |       await expect(this.nonVegeterian).toBeVisible({
  258 |         timeout: 5000,
  259 |       });
  260 |     }
  261 | 
  262 |     // Select Non-Vegetarian
  263 |     await this.safeClick(this.nonVegeterian);
  264 | 
  265 |     // =====================================================
  266 |     // PRICE
  267 |     // =====================================================
  268 | 
  269 |     await this.item_price.click();
  270 | 
  271 |     await this.item_price.fill(price);
  272 | 
  273 |     // =====================================================
  274 |     // CATEGORY
  275 |     // =====================================================
  276 | 
  277 |     await this.safeClick(this.categoryDropdown);
  278 | 
  279 |     await expect(this.drinks).toBeVisible({
  280 |       timeout: 10000,
  281 |     });
  282 | 
  283 |     // Select Drinks
  284 |     await this.safeClick(this.drinks);
  285 | 
  286 |     // Dropdown may close after first selection
  287 |     if (!(await this.allDesert.isVisible())) {
  288 |       await this.safeClick(this.categoryDropdown);
  289 | 
  290 |       await expect(this.allDesert).toBeVisible({
  291 |         timeout: 5000,
  292 |       });
  293 |     }
  294 | 
  295 |     // Select All Desert
  296 |     await this.safeClick(this.allDesert);
  297 | 
  298 |     // =====================================================
  299 |     // SUB CATEGORY
  300 |     // =====================================================
  301 | 
  302 |     await this.safeClick(this.subCategoryDropdown);
  303 | 
  304 |     await this.scrollAndSelectSubCategory("Strawberry with Belgian chocolate");
  305 | 
  306 |     // Dropdown may close after first selection
  307 |     await this.safeClick(this.subCategoryDropdown);
  308 | 
  309 |     await this.scrollAndSelectSubCategory("Milkshakes");
  310 | 
  311 |     // =====================================================
  312 |     // FILE UPLOAD
  313 |     // =====================================================
  314 | 
  315 |     await this.fileInput.setInputFiles(
  316 |       "E:/Automation_Testing/Tawla_Automation/utils/png-transparent-full-breakfast-british-cuisine-english-cuisine-breakfast-sausage-woodbury-commons-outlet-food-breakfast-recipe-thumbnail.png",
  317 |     );
  318 | 
  319 |     // =====================================================
  320 |     // DESCRIPTION
  321 |     // =====================================================
  322 | 
  323 |     await this.itemDescription.fill(generateFoodDescription());
  324 | 
  325 |     // =====================================================
  326 |     // SAVE ITEM
  327 |     // =====================================================
  328 | 
  329 |     await this.page
  330 |       .getByRole("button", {
  331 |         name: "Save Item",
  332 |       })
  333 |       .click();
  334 | 
  335 |     // DEBUG
  336 |     // await this.page.pause();
  337 |   }
  338 | }
  339 | 
  340 | module.exports = { items };
  341 | 
```