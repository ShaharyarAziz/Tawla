...
Call log:
  - waiting for getByText('Categories') to be visible
    2 × locator resolved to hidden <p>Deleting a category will also remove all of its s…</p>
...
```ts
  ...
  16 |         (resp) =>
  17 |           resp.url().includes("digitalMenuSection") &&
  18 |           resp.status() === 200
  19 |       ),
  20 |       this.digitalMenuLink.click(),
  21 |     ]);
  22 | 
  23 |     // Strong UI validation (very important)
> 24 |     await this.page.getByText("Categories").waitFor({ timeout: 20000 });
     |                                             ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  25 |   }
  26 | }
```