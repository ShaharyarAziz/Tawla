...
test('User pays and serves order from Order Management', async ({ page }) => {
  ...
  const wasPaid = await orderManagement.paidText.isVisible().catch(() => false);
  const payVisible = await orderManagement.payForOrderButton.isVisible().catch(() => false);
  const canPay = payVisible
    ? await orderManagement.payForOrderButton.isEnabled().catch(() => false)
    : false;
  let shouldServe = wasPaid;
